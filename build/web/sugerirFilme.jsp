...
<body class="bg-dark text-white index-body">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark">
        <div class="container-fluid">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="home.jsp" class="btn btn-outline-info ms-2">Voltar</a>
            </div>
        </div>
    </nav>
    ...
    <div class="container mt-5">
        ...
        <div class="row">
                ...
                <div class="card filme-card-dreamcore text-white shadow-lg h-100">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <% if (f.getDescricao() != null && !f.getDescricao().trim().isEmpty()) {
                            ...
                            String resumo = desc.length() > 150 ? desc.substring(0, 150) + "..." : desc;
                        %>
                            <p class="card-text mb-1"><small><strong>Descrição:</strong> <%= resumo %></small></p>
                        <% } %>
                        ...
                        <div class="mt-auto pt-3 d-grid gap-2 d-sm-flex justify-content-sm-between">
                            <% if (usuario.getId() == f.getUsuarioId() || "admin".equals(usuario.getTipo())) { %>
                                <a href="FilmeServlet?action=editar&id=<%= f.getId() %>&returnPage=sugerir" class="btn btn-outline-light btn-sm">
                                    <i class="bi bi-pencil-square"></i> Editar
                                </a>
                                <a href="FilmeServlet?action=excluir&id=<%= f.getId() %>&returnPage=sugerir" class="btn btn-outline-danger btn-sm" onclick="return confirm('Tem certeza?')">
            ...
            <% } } else { %>
                <p class="text-center typing glow-text">Nenhum filme sugerido ainda!</p>
            <% } %>
        </div>

        <hr class="text-white my-5">
        <div class="text-center mb-4">
            <button type="button" class="btn btn-outline-info btn-lg fw-bold" data-bs-toggle="modal" data-bs-target="#sugestaoFilmeModal">
    ...
    <!-- Modal Sugerir -->
    <div class="modal fade" id="sugestaoFilmeModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title glow-text">🌌 Sugira um novo filme</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form action="FilmeServlet" method="post">
                        <input type="hidden" name="action" value="sugerir">
                        <input type="hidden" name="returnPage" value="sugerir">
                        ...
                        <div class="mb-3"><label class="form-label">Descrição</label><textarea class="form-control" name="descricao" rows="3"></textarea></div>
                        <div class="mb-3"><label class="form-label">Link do Trailer(Opcional)</label><input 
                                    type="url"</label><input type="url" class="form-control" name="trailerLink"></div>
                        <div class="d-grid"><button type="submit" class="btn btn-outline-info btn-lg fw-bold">📤 Enviar sugestão</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <!-- Modal Editar -->
    <div class="modal fade" id="editFilmeModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title glow-text">✏️ Editar Sugestão</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form id="formEditFilme" action="FilmeServlet" method="post">
                        <input type="hidden" name="action" value="atualizar">
                        <input type="hidden" id="editFormId" name="id">
                        <input type="hidden" name="returnPage" value="sugerir">
                        <div class="mb-3"><label class="form-label">Título</label><input type="text" class="form-control" id="editFormTitulo" name="titulo" required></div>
                        <div class="mb-3"><label class="form-label">Gênero</label><input type="text" class="form-control" id="editFormGenero" name="genero" required></div>
                        <div class="mb-3"><label class="form-label">Nota</label><input type="number" class="form-control" id="editFormNota" name="nota" min="0" max="10" step="0.1" required></div>
                        <div class="mb-3"><label class="form-label">Descrição</label><textarea class="form-control" id="editFormDescricao" name="descricao" rows="3"></textarea></div>
                        <div class="mb-3"><label class="form-label">Link do Trailer(Opcional)</label><input 
                                    type="url"</label><input type="url" class="form-control" id="editFormTrailer" name="trailerLink"></div>
    ...
    <footer class="text-center text-light py-3 mt-auto">
        ...
        <small>© 2025 Meu Cérebro, Meu Universo — desenvolvido com criatividade e ciência por RAISSA ANUNCIAÇÃO & HECTOR BERNHARDT.</small>
    </footer>
    ...
    <% if (filmeParaEditar != null) { %>
        ...
        const formTitulo = document.getElementById("editFormTitulo");
        const formGenero = document.getElementById("editFormGenero");
        const formNota = document.getElementById("editFormNota");
        const formDescricao = document.getElementById("editFormDescricao");
        const formTrailer = document.getElementById("editFormTrailer");
        const filmeModal = new bootstrap.Modal(document.getElementById("editFilmeModal"));
